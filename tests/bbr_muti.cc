...
void ThroughputMonitor (FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon, std::string file_prefix_name)
	{
        //double localThrou=0;
		...
		for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator stats = flowStats.begin (); stats != flowStats.end (); ++stats)
		{
			...
			if(fiveTuple.sourceAddress == "10.1.1.1")
			{
				...
				outFile.open(file_prefix_name + "throughput_1.csv", ios::out | ios::app); 
			...
			{
				std::ofstream outFile;
				outFile.open(file_prefix_name + "throughput_2.csv", ios::out | ios::app);
			    outFile << Simulator::Now().GetSeconds() << ',' << stats->second.rxBytes * 8.0 / (stats->second.timeLastRxPacket.GetSeconds()-stats->second.timeFirstTxPacket.GetSeconds())/1024/1024 << endl;				
				outFile.close();
			}
		}
			Simulator::Schedule(Seconds(0.001),&ThroughputMonitor, fmhelper, flowMon, file_prefix_name);
   //if(flowToXml)
      //{
...
int main (int argc, char *argv[]) {
  ...
  std::string S_TO_R_BW = "10Mbps";
  std::string S_TO_R_DELAY = "10ms";
  std::string R_TO_C_BW = "10Mbps";
  ...
  Config::SetDefault( "ns3::RttEstimator::InitialEstimation", TimeValue( MilliSeconds( 30 ) ) );
  ...
  std::string file_prefix_name = S_TO_R_BW + "_" + R_TO_C_BW + "_" + std::to_string(q_disc_size) + "-";
  ...
  NodeContainer senders, routers, receivers, nodes;  // 0=source1, 1=source2, 2=router, 3=sink
  senders.Create(nSenders);
  routers.Create(1);
  receivers.Create(1);
  nodes.Add(senders, routers, receivers);
  ...
  */
  ...
  // Server to Router.
  int mtu = 1500;
  PointToPointHelper p2pS2R, p2pR2C;
  p2pS2R.SetDeviceAttribute("DataRate", StringValue (S_TO_R_BW));
  p2pS2R.SetChannelAttribute("Delay", StringValue (S_TO_R_DELAY));
  p2pS2R.SetDeviceAttribute ("Mtu", UintegerValue(mtu));

  NetDeviceContainer s2r;
  for( uint32_t i = 0; i < nSenders; i++)
  {
    //NetDeviceContainer s2r = p2pS2R.Install(senders.Get(i), routers.Get(0));
    s2r.Add(p2pS2R.Install(senders.Get(i), routers.Get(0)));
  }
  ...
  // Router to Client.
  p2pR2C.SetDeviceAttribute("DataRate", StringValue (R_TO_C_BW));
  p2pR2C.SetChannelAttribute("Delay", StringValue (R_TO_C_DELAY));
  p2pR2C.SetDeviceAttribute ("Mtu", UintegerValue(mtu));
  NS_LOG_INFO("Router queue size: "<< QUEUE_SIZE);
  p2pR2C.SetQueue("ns3::DropTailQueue",
               "Mode", StringValue ("QUEUE_MODE_PACKETS"),
               "MaxPackets", UintegerValue(QUEUE_SIZE));
  NetDeviceContainer r2c = p2pR2C.Install(routers.Get(0), receivers.Get(0));
  ...
  Config::SetDefault("ns3::RedQueueDisc::MinTh", DoubleValue(q_disc_size));
  Config::SetDefault("ns3::RedQueueDisc::MaxTh", DoubleValue(q_disc_size));
  Config::SetDefault("ns3::RedQueueDisc::Gentle", BooleanValue(false));
  QueueDiscContainer qdiscs = tch.Install (r2c.Get(0));
  ...
  Ipv4InterfaceContainer i0i2 = ipv4.Assign(s2r);
  
  //ipv4.SetBase("10.2.1.0", "255.255.255.0");
  //Ipv4InterfaceContainer i1i2 = ipv4.Assign(devices2);

  ipv4.SetBase("192.168.1.0", "255.255.255.0");
  Ipv4InterfaceContainer i2i3 = ipv4.Assign(r2c);
  ...
  // Source (at node 0).
  for (u_int32_t i = 0, i <= nSenders, i++)
  {
    ...
    {
      apps.Start(Seconds(START_TIME + 3 + i*10));
    }
...
/* 
  ...
  ApplicationContainer s1apps = source1.Install(nodes.Get(0));
  s1apps.Start(Seconds(START_TIME));
  s1apps.Stop(Seconds(STOP_TIME));
  ...
  AoolicationContainer s2apps = source2.Install(nodes.Get(1));
  s2apps.Start(Seconds(23));
  s2apps.Stop(Seconds(STOP_TIME));
*/
  ...
  AsciiTraceHelper ath;
  Ptr<Queue<Packet> > queue = StaticCast<PointToPointNetDevice> (r2c.Get (0))->GetQueue ();
  Ptr<OutputStreamWrapper> streamPacketsInQueue = ath.CreateFileStream (file_prefix_name + queueDiscType + "-PacketsInDeviceQueue.csv");
  queue->TraceConnectWithoutContext ("PacketsInQueue",MakeBoundCallback (&PacketsInQueueTrace, streamPacketsInQueue));

  Ptr<QueueDisc> q = qdiscs.Get(0);
  Ptr<OutputStreamWrapper> streamPacketsInQueueDisc = ath.CreateFileStream (file_prefix_name + queueDiscType + "-PacketsInQueueDisc.csv");
  q->TraceConnectWithoutContext ("PacketsInQueue", MakeBoundCallback (&TcPacketsInQueueTrace, streamPacketsInQueueDisc));

/* 
  // Trace changes to the congestion window
  Ptr<OutputStreamWrapper> CwndChanges_n0 = ath.CreateFileStream (file_prefix_name + "CwndChanges_n0.csv");
  Ptr<OutputStreamWrapper> CwndChanges_n1 = ath.CreateFileStream (file_prefix_name + "CwndChanges_n1.csv");
  ...
  Ptr<OutputStreamWrapper> Rtt_n0 = ath.CreateFileStream (file_prefix_name + "Rtt_n0.csv");
  Ptr<OutputStreamWrapper> Rtt_n1 = ath.CreateFileStream (file_prefix_name + "Rtt_n1.csv");
...
*/
  ...
  ThroughputMonitor(&fmHelper, allMon, file_prefix_name);
