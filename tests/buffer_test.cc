...
int main (int argc, char *argv[]) {
  ...
  Ptr<OutputStreamWrapper> streamPacketsInQueue = ath.CreateFileStream (queueDiscType + "-PacketsInDeviceQueue.txt");
  queue->TraceConnectWithoutContext ("PacketsInQueue",MakeBoundCallback (&PacketsInQueueTrace, streamPacketsInQueue));

  Ptr<QueueDisc> q = qdiscs.Get(0);
  Ptr<OutputStreamWrapper> streamPacketsInQueueDisc = ath.CreateFileStream (queueDiscType + "-PacketsInQueueDisc.txt");
