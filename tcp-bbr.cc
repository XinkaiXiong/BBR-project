...
void TcpBbr::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t packets_acked,
  ...
  double alpha = rtt.GetSeconds()/min_rtt.GetSeconds();
  if (m_pacing_gain == 1.25){
    if (alpha >= 1.25){
      m_pacing_gain = 0.75;
    }
  }
  if (m_pacing_gain == 1){
    m_pacing_gain = 0.75/alpha;
  }
  ...
  NS_LOG_INFO(this << "  DATA rtt: " << rtt.GetSeconds() << "  " <<
              ...
              "bw: " << bw_est << " Mb/s  " <<
              "max_bw: " << max_bw << " Mb/s  " <<
              "bytes_in_flight: " << m_bytes_in_flight << " bytes  " <<
              "m_cwnd: " << m_cwnd << " bytes  " <<
              "tcb->m_cWnd: " << tcb->m_cWnd << " bytes  " <<
              "min_rtt: " << min_rtt.GetSeconds());
}
...
void TcpBbr::Send(Ptr<TcpSocketBase> tsb, Ptr<TcpSocketState> tcb,
  ...
  m_bytes_in_flight = tsb -> AdjBytesInFlight();
